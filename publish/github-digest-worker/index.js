...
function pickLatestNewsItem(items, now) {
  const sorted = [...items].sort((a, b) => {
    const aTime = new Date(a.pubDate || a.title).getTime();
    const bTime = new Date(b.pubDate || b.title).getTime();
    return bTime - aTime;
  });

  for (const item of sorted) {
    const itemTime = new Date(item.pubDate || item.title);
...
async function executeDeepSeekAttempt(env, options, attempt) {
  ...
  const response = await fetch(`${String(env.DEEPSEEK_BASE_URL || DEEPSEEK_API_BASE)}/chat/completions`, {
    method: "POST",
    headers: {
